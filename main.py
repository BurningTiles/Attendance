...
def markAttendance(data):
    with open('Attendance.csv', 'r+') as f:
        ...
        if enrollment not in nameList:
            ...
            db.reference('Days').child(now.strftime('%d%m%Y')+'/'+enrollment).set({'time': now.strftime(currentTime)})
            db.reference('Students').child(enrollment).push({'date': now.strftime('%d %B %Y'), 'time': now.strftime(currentTime)})
